# Lab1 实验：基于大模型的编程与Git实战  可视化界面 Changes on B1
...
def main():
    ...
    root = tk.Tk()
    root.withdraw()
    filepath = filedialog.askopenfilename(title="请选择输入文本文件", filetypes=[("Text Files", "*.txt"), ("All Files", "*.*")])
    if not filepath:
    ...
    # 结果显示区
    result_text = scrolledtext.ScrolledText(result_frame, width=60, height=18, state='normal', font=("Consolas", 11))
